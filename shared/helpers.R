...
# 固定材料位置：将指定材料在重复2+中的位置修正为与重复1相同
# fixed_names: 用户输入的原始名称（与 source 列匹配，回退到 name 列）
# 注：code 现在随材料（=stageid数字部分），用行索引找目标位置
fixMaterialPositions <- function(planted_df, fixed_names) {
  if (length(fixed_names) == 0 || !"rp" %in% names(planted_df)) {
    return(planted_df)
  }
  rps <- unique(planted_df$rp)
  if (length(rps) <= 1) {
    message("[fixMaterialPositions] 只有1个重复，无需固定")
    return(planted_df)
  }

  has_source <- "source" %in% names(planted_df)
  message("[fixMaterialPositions] 固定材料: ", paste(fixed_names, collapse=", "))
  message("[fixMaterialPositions] 数据列: source=", has_source, " name=TRUE rp=TRUE nrow=", nrow(planted_df))

  # 查找材料在某重复中的全局行号
  find_by <- function(df, rp_val, search_name) {
    sn <- trimws(search_name)
    if (has_source) {
      hits <- which(df$rp == rp_val & trimws(as.character(df$source)) == sn)
      if (length(hits) == 1) {
        message("[fixMaterialPositions]   rp=", rp_val, " 匹配 '", sn, "' via source")
        return(hits)
      }
    }
    hits <- which(df$rp == rp_val & trimws(as.character(df$name)) == sn)
    if (length(hits) == 1) {
      message("[fixMaterialPositions]   rp=", rp_val, " 匹配 '", sn, "' via name")
    }
    hits
  }

  # 各重复对应的行号（全局）
  rp_idx <- split(seq_len(nrow(planted_df)), planted_df$rp)

  fixed_count <- 0
  for (rpi in rps[rps != 1]) {
    for (fn in fixed_names) {
      rep1_hit <- find_by(planted_df, 1L, fn)
      if (length(rep1_hit) != 1) {
        message("[fixMaterialPositions] 警告: '", fn, "' 在重复1中未找到")
        next
      }
      # 材料在rep1中的位置（第几行）
      rep1_pos <- which(rp_idx[[as.character(1L)]] == rep1_hit)

      curr <- find_by(planted_df, rpi, fn)
      if (length(curr) != 1) {
        message("[fixMaterialPositions] 警告: '", fn, "' 在重复", rpi, "中未找到")
        next
      }

      # 目标：rep rpi 中相同位置的行
      target <- rp_idx[[as.character(rpi)]][rep1_pos]
      if (is.na(target)) {
        message("[fixMaterialPositions] 警告: 重复", rpi, "中没有位置 ", rep1_pos, " 的行")
        next
      }
      if (curr == target) {
        message("[fixMaterialPositions] '", fn, "' 在重复", rpi, "中已在同一位置，无需移动")
        next
      }

      # 交换整行（含code，因为code现在也是材料属性）
      tmp <- planted_df[target, ]
      planted_df[target, ] <- planted_df[curr, ]
      planted_df[curr, ] <- tmp
      fixed_count <- fixed_count + 1
      message("[fixMaterialPositions] 固定: '", fn, "' 重复", rpi, " 从行", curr, " -> 行", target)
    }
  }
  message("[fixMaterialPositions] 完成: 共固定 ", fixed_count, " 个材料位置")
  planted_df
}
