buildDesignplotServer <- function(input, output, session) {
  ...
  # 种植快照 — 持久化快照（双槽位）
  # ===========================================================================
  snapshotInfo <- function(slot) {
    snapshotTrigger()
    getPlantingSnapshotInfo(slot, sqlite_db_path)
  }

  buildSnapshotStatus <- function(slot) {
    info <- snapshotInfo(slot)
  ...
  output$snapshot1StatusUi <- renderUI({ buildSnapshotStatus(1L) })
  output$snapshot2StatusUi <- renderUI({ buildSnapshotStatus(2L) })
  ...
  snapshotSaveImpl <- function(slot) {
    ...
    savePlantingSnapshot(slot, exp_id$experiment_id, pt, ck, "", sqlite_db_path)
  ...
  makeSnapshotObservers <- function(slot) {
    s <- as.character(slot)
    saveInput <- paste0("snapshot", s, "Save")
    restoreInput <- paste0("snapshot", s, "Restore")
    confirmRestore <- paste0("confirmSnapshot", s, "Restore")
    clearInput <- paste0("snapshot", s, "Clear")
    confirmClear <- paste0("confirmSnapshot", s, "Clear")

    observeEvent(input[[saveInput]], {
      tryCatch({
        snapshotSaveImpl(slot)
        showNotification(paste0("快照 ", s, " 已保存"), type = "message")
      }, error = function(e) {
        showNotification(paste0("保存快照 ", s, " 失败: ", e$message), type = "error")
      })
    })

    observeEvent(input[[restoreInput]], {
      if (!hasPlantingSnapshot(slot, sqlite_db_path)) {
        showNotification(paste0("快照 ", s, " 为空"), type = "warning")
        return(invisible(NULL))
      }
      showModal(modalDialog(
        title = tagList(icon("exclamation-triangle"), paste0("确认恢复快照 ", s)),
        tags$p(paste0("将恢复快照 ", s, " 的种植状态，覆盖当前地块数据。该操作不可撤销。"),
               style = "margin:0;color:#b91c1c;"),
        footer = tagList(
          modalButton("取消"),
          actionButton(confirmRestore, "确认恢复", class = "btn-danger")
        ),
        easyClose = TRUE
      ))
    })

    observeEvent(input[[confirmRestore]], {
      tryCatch({
        removeModal()
        snapshotRestoreImpl(slot)
        showNotification(paste0("快照 ", s, " 已恢复"), type = "message")
      }, error = function(e) {
        showNotification(paste0("恢复快照 ", s, " 失败: ", e$message), type = "error")
      })
    })

    observeEvent(input[[clearInput]], {
      if (!hasPlantingSnapshot(slot, sqlite_db_path)) {
        showNotification(paste0("快照 ", s, " 已为空"), type = "warning")
        return(invisible(NULL))
      }
      showModal(modalDialog(
        title = paste0("确认清除快照 ", s),
        tags$p("清除后不可恢复。", style = "margin:0;"),
        footer = tagList(
          modalButton("取消"),
          actionButton(confirmClear, "确认清除", class = "btn-danger")
        ),
        easyClose = TRUE
      ))
    })
    ...
    observeEvent(input[[confirmClear]], {
      tryCatch({
        removeModal()
        snapshotClearImpl(slot)
        showNotification(paste0("快照 ", s, " 已清除"), type = "message")
      }, error = function(e) {
        showNotification(paste0("清除快照 ", s, " 失败: ", e$message), type = "error")
      })
    })
  }

  makeSnapshotObservers(1L)
  makeSnapshotObservers(2L)
  ...
  fieldLayoutData <- reactive({
    ...
    tryCatch({
      ...
      # 读取 experiment_id -> experiment_name 映射
      exp_names_df <- DBI::dbGetQuery(con,
        "SELECT experiment_id, experiment_name FROM designplot_experiments")

      # 构建 experiment_id -> experiment_name 命名向量
      exp_id_to_name <- if (is.data.frame(exp_names_df) && nrow(exp_names_df) > 0) {
        stats::setNames(as.character(exp_names_df$experiment_name), as.character(exp_names_df$experiment_id))
      } else {
        character()
      }
      ...
      if (length(valid_exp_ids) > 0 && is.data.frame(plan_runs_df) && nrow(plan_runs_df) > 0) {
        ...
        if (is.data.frame(assignments_df) && nrow(assignments_df) > 0) {
          ...
          # plant 表数据列：V1..V200 对应 col 1..200
          col_seq_map <- stats::setNames(as.integer(assignments_df$field_col_no),
                                         as.integer(assignments_df$seq_no))
          row_seq_map <- stats::setNames(as.integer(assignments_df$field_row_no),
                                         as.integer(assignments_df$seq_no))
          seq_exp_map <- stats::setNames(as.character(assignments_df$experiment_name),
                                         as.integer(assignments_df$seq_no))
          ...
          # 补种识别：用 plant 表里 parseAssignmentCell 能解析但不在 known_mat_names 里的
          ...
          for (r in seq_len(nrow(plant_data))) {
            for (c in seq_len(ncol(plant_data))) {
              ...
              if (mat_name %in% known_mat_names) next
              key <- paste0("补种:", mat_name)
              if (is.null(supplement_cell_map[[key]])) {
                supplement_cell_map[[key]] <- matrix(FALSE, nrow = nrow(plant_data), ncol = ncol(plant_data))
              }
      ...
      if (length(layout_info) > 0) {
        ...
        if (length(exp_names) > 0) {
          # 按实验名字典序排序，保证每次生成一致的调色板顺序
          exp_names_sorted <- sort(exp_names)
          # 使用固定调色板（蓝绿紫橙粉）循环，避免随机跳色
          base_palette <- c("#4E79A7", "#F28E2B", "#59A14F", "#E15759",
                            "#76B7B2", "#EDC948", "#B07AA1", "#FF9DA7",
                            "#9C755F", "#BAB0AC")
          palette_idx <- ((seq_along(exp_names_sorted) - 1) %% length(base_palette)) + 1
          exp_colors <- base_palette[palette_idx]
          names(exp_colors) <- exp_names_sorted
          exp_idx <- which(result$type == "实验")
          result$color[exp_idx] <- as.character(exp_colors[as.character(result$name[exp_idx])])
        }
