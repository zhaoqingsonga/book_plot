...
savePlantingSnapshot <- function(slot, experiment_id, plant_table_name, checkpoint_list,
  ...
  ", params = list(slot, experiment_id, plant_table_name, snapshot_label, list(blob_data)))

  invisible(TRUE)
}

#' 从指定槽位加载快照（返回 checkpoint list，无快照时返回 NULL）
...
#' 获取指定槽位的快照状态信息
getPlantingSnapshotInfo <- function(slot, db_path = defaultSqlitePath()) {
  slot <- as.integer(slot)
  if (!slot %in% c(1L, 2L)) stop("snapshot_slot 必须为 1 或 2")
  ...
  row <- DBI::dbGetQuery(con, "
    ...
    FROM planting_snapshots WHERE snapshot_slot = ?
  ", params = list(slot))
