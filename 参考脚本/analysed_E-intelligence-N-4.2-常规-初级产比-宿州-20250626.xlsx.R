...
# 主要参数
# ==============================================================================
file_name <-"analysed_E-intelligence-N-4.2-常规-初级产比-宿州-20250626.xlsx"
sheet_name = "promotion"

#不晋级材料
keep_eliminated_vector<-c("N25E133")#为皖豆37
#不选单株材料
...
screened<-screen_material_promotion(
    ...
    rank_threshold_select = 100,    # 晋级材料筛选阈值（原rank_threshold）
    rank_threshold_plant = 180,     # 高产分离选单株筛选阈值（新增独立控制）
...
excellent<-analyze_and_save_parents_crosses(
    ...
    output_directory,
    PLACE,
    parent_min_crosses = 3,#亲本最小组合数
    parent_top_pct = 0.7,#前
    cross_min_crosses = 2,#组合最小数
    cross_promote_rate_thresh = 0.2,#组合晋级率
    cross_yield_threshold_adjust = 0.5#产量调节
)
...
plot_analysis_results(
  ...
  parent_label_vjust = 0,       # 优化亲本标签垂直偏移
  cross_label_vjust = 2,
  width=12,
  height=15
  )        # 新增：柱状图标签垂直偏移     
